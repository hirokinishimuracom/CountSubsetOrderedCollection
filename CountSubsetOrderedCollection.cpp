...
int main() {
  ...
  std::cout
      << "The construction of the power set of X is complete. It contains "
      << P.size() << " subsets of X.\n\n";
...
int count_subset(int k, std::set<std::set<int>> C,
  ...
  int num = 0;
  for (auto it_P = P_sorted[k - 1].begin(); it_P != P_sorted[k - 1].end();
       ++it_P) {
    for (auto it_C = C.begin(); it_C != C.end(); ++it_C) {
      if (std::includes(it_C->begin(), it_C->end(), it_P->begin(),
                        it_P->end())) {
        ++num;
        break;
      }
...
std::vector<std::set<int>> order_Pk(
  ...
  int ans;
  std::cout << "Type any-nonzero integer to use the lex min-min or 0 to use "
               "the lex min-max: ";
  std::cin >> ans;
  std::cout << "\n";
  if (ans) {
